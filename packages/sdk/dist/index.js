...
var FingerprintJS__default = /*#__PURE__*/_interopDefault(FingerprintJS);
...
var KEYGUARD_STORE = idbKeyval.createStore("keyguard-db", "keyguard-store");
var STORAGE_KEYS = {
...
var BrowserStorageAdapter = class {
  ...
  async saveKeyPair(publicKey, privateKey) {
    try {
      ...
      await idbKeyval.set(STORAGE_KEYS.PUBLIC_KEY, publicKey, KEYGUARD_STORE);
      await idbKeyval.set(STORAGE_KEYS.PRIVATE_KEY, privateKey, KEYGUARD_STORE);
    } catch (error) {
      throw new Error(
  ...
  async getKeyPair() {
    try {
      const publicKey = await idbKeyval.get(STORAGE_KEYS.PUBLIC_KEY, KEYGUARD_STORE);
      const privateKey = await idbKeyval.get(STORAGE_KEYS.PRIVATE_KEY, KEYGUARD_STORE);
  ...
  async clear() {
    try {
      await idbKeyval.del(STORAGE_KEYS.PUBLIC_KEY, KEYGUARD_STORE);
      await idbKeyval.del(STORAGE_KEYS.PRIVATE_KEY, KEYGUARD_STORE);
    } catch (error) {
      throw new Error(
...
async function getDeviceFingerprint() {
  try {
    const fpPromise = FingerprintJS__default.default.load();
